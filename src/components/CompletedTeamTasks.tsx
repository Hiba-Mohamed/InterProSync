...
const CompletedTeamTasks = ({
  teamCompletedTasks,
}: {
  teamCompletedTasks: CompletedTasksDisplayObjectType[];
}) => {
    ...
    <Box>
      ...
      {teamCompletedTasks.length === 0 ? (
        <Typography variant="h6" textAlign="center" color="gray">
          No completed tasks for other teams.
        </Typography>
      ) : (
        teamCompletedTasks.map((task, index) => (
          <Box
            key={index}
            sx={{
              marginBottom: 3,
...
export default CompletedTeamTasks;
